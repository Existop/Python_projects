...
def draw_hit_blocks(hit_blocks_for_draw_from):
    """Draws 'X' in the blocks that were successfully hit either by computer or by human
    """
    for block in hit_blocks_for_draw_from:
...
def show_message_at_rect_center(message, rect, which_font=font, color=RED):
    text_width, text_hight = which_font.size(message)
    text_rect = pygame.Rect(rect)
    x_start = text_rect.centerx - text_width / 2
    y_start = text_rect.centery - text_hight / 2
    background_rect = pygame.Rect(
        x_start - block_size / 2, y_start, text_width + block_size, text_hight)
    text_to_blit = which_font.render(message, True, color)
...
def check_ship_numbers(ship, num_ship_list):
...
def main():
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if human_ships_to_draw:
                    ...
                    num_ship_list[len(deleted_ship) - 1] -= 1
                    update_used_blocks(
                ...
                if 15 < start_block[0] < 26 and 0 < start_block[1] < 11 and 15 < end_block[0] < 26 and 0 < end_block[1] < 11:
                    ...
                    if start_block[0] == end_block[0] and (end_block[1] - start_block[1]) < 4:
                        ...
                        show_message_at_rect_center(
                            "КАРАБЛЬ СЛИШКОМ БОЛЬШОЙ", message_rect_for_drawing_ships)
                else:
                    show_message_at_rect_center(
                ...
                if temp_ship:
                    ...
                    if ship_is_valid(temp_ship_set, used_blocks_for_manual_drawing):
                        if check_ship_numbers(temp_ship, num_ship_list):
                            num_ship_list[len(temp_ship) - 1] += 1
    ...
    while not game_over:
        ...
        if computer_turn:
            ...
            show_message_at_rect_center(f"ПОСЛЕДНИЙ ХОД КОМПЬЮТЕРА: {LETTERS[fired_block[0] - 16] + str(fired_block[1])}", message_rect_human, color=BLACK)
