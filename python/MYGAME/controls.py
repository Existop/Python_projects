...
def events(screen, gun, bullets):  #Обработка событий
     for event in pygame.event.get():
            if event.type == pygame.QUIT:
                sys.exit()
            elif event.type == pygame.KEYDOWN: # вправо кнопка
            ...
            elif event.type == pygame.KEYUP:  # вправо кнопка
                if event.key == pygame.K_d:
                        gun.mright = False
                elif event.key == pygame.K_a:
                        gun.mleft = False
...
def update(bg_color, screen, stats, sc, gun, inos, bullets):
    ...
    gun.output()
    inos.draw(screen)
    pygame.display.flip() # Прорисовать посл екран , например вывод очков
...
def inos_check(stats, screen, sc, gun, inos, bullets):
    # Побралась ли армия до края екрана
...
def check_high_score(stats, sc):
    ...
    if stats.score > stats.high_score:
        ...
        sc.image_high_score()
        with open('MYGAME/highscore.txt', 'w') as f:
